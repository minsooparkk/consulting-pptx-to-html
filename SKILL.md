...
description: "완성된 PPTX를 문구·장수·순서·좌표·글꼴·색상·마스터·Header·Footer를 최대한 보존하는 HTML 발표 자료로 변환한다. PPTX 디자인 유지, PPT를 HTML 강의안으로 변환, consulting-pptx 결과의 웹 발표 버전 제작 요청에 사용한다."
---
...
2. 스킬 폴더를 기준으로 변환한다. Python 3.9 이상, 표준 라이브러리만 필요하다.
   ...
   ```text
   python scripts/convert.py input.pptx -o output.html
   ```

3. 함께 생성되는 `output.audit.json`을 읽는다. `serialization_check`, `slide_count`, `issues`, `stats`, `fonts_requested`, `notes_included`를 확인한다.
...
5. 글꼴을 확인한다. 기본 출력은 CDN/네트워크 요청이 없고 시스템 글꼴을 쓴다. 글꼴이 없으면 설치하거나, 재배포 권한이 있는 파일만 포함한다. 폰트 이름 지정만으로 실제 설치를 확인했다고 말하지 않는다.
...
6. HTML을 브라우저로 열고 모든 슬라이드의 텍스트 넘침·경계, 이동·전체화면·편집·저장·인쇄를 확인한다. `window.PPTPlayer.audit()` 또는 다음 검사기를 사용한다.

   ```text
   python scripts/check_html.py output.html --report browser-qa.json
   ```

7. PowerPoint 원본과 HTML을 같은 크기로 나란히 비교한다. 소스 렌더러가 없으면 `PowerPoint 시각 대조 미실시`라고 분명히 알린다. HTML 화면 검사와 PowerPoint 일치 검사를 구분한다. 경고를 없애기 위해 문구를 고치거나 원본 글꼴 크기를 줄이지 않는다.
...
- 퀴즈, 계산기, 단계별 공개 같은 추가 동작은 별도 요청이 있을 때 HTML에 추가한다. 기본 변환에 자동 적용하지 않는다.
