...
- `--motion auto`가 기본이다. 각 슬라이드 진입 시 내용을 자동 표시한다. 요소별 재생은 420ms, 지연은 최대 500ms로 전체 내용이 약 0.92초 안에 나타나게 한다. 슬라이드 자체는 자동으로 넘기지 않는다.
- 방향키·Space·이전/다음 버튼은 한 번 조작할 때 한 슬라이드 이동한다. 다음 항목 공개에 이동 키를 소모하지 않는다. 클릭 강조, 수동 단계 버튼, 별도 재생 버튼을 기본으로 추가하지 않는다.
- 정적인 결과를 원하면 `--motion none`을 쓴다. 모션 감소 설정, 편집·저장·인쇄·검수에서는 내용이 숨은 채 남지 않도록 한다.
